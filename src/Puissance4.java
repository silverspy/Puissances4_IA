...
public class Puissance4 {
    ...
    public Puissances4() {
        ...
        this.input_board=new String[6][7];
        this.output_board = new String[6][7];
    }
    //ok
    public void exportGame() {
        String str="";
        for(int i=(this.output_board.length-1);i>=0;i--) {
            for (int j = 0; j < this.output_board[i].length; j++) {
                str += this.output_board[i][j];
            }
            str += "\n";
        }
    ...
    public  void changementJeux(String nomFile){
        ...
        for(int y=0;y<this.input_board.length;y++) {
            t[y]=t[y].replace("X", "D");
            t[y]=t[y].replace("O", "X");
            t[y]=t[y].replace("D", "O");
            char[] lineChar=t[y].toCharArray();
            for(int x=0;x<this.input_board[y].length;x++) {
                this.input_board[y][x]=String.valueOf(lineChar[x]);
            }
        }
        for(int i=(this.input_board.length-1);i>=0;i--) {
            for(int j=0;j<this.input_board[i].length;j++) {
                this.output_board[i][j]=this.input_board[i][j];
            }
        }
    ...
    public void play(int x,int y, boolean player) {
        if(player)
        {
            this.input_board[x][y]="O";
        }
        else
        {
            this.input_board[x][y]="X"
        }
    }

    public void isEmpty(int x, int y)
    {
        if(this.input_board[x][y].equals("-"))
        {
            return true;
        }
        else
    ...
    public void bestPlay2(String [][] plateauCourant,HashMap<int[],Integer> accuValue,int recursivityLevel,int branchKey,List<int[]> listBranchKey) {
        if(branchKey==2&&recursivityLevel>=3) {
            ...
            playBestScore(accuValue, this.input_board);
            return;
        }else {
            ...
            if(accuValue.containsValue(17)||accuValue.containsValue(16)||accuValue.get(listBranchKey.get(branchKey))==12||accuValue.get(listBranchKey.get(branchKey))==11||accuValue.get(listBranchKey.get(branchKey))==10||accuValue.get(listBranchKey.get(branchKey))==9) {
                plateauCourant=simulationCoup(this.input_board,listBranchKey.get(branchKey+1), "O");
                ...
                if(accuValue.get(listBranchKey.get(branchKey))==-17||accuValue.get(listBranchKey.get(branchKey))==-12||accuValue.get(listBranchKey.get(branchKey))==-11||accuValue.get(listBranchKey.get(branchKey))==-10||accuValue.get(listBranchKey.get(branchKey))==-9) {
                    ...
                    if(recursivityLevel%2==0) {
                        if(recursivityLevel!=6) {
                            ...
                            String[][] plateauAdv=simulateCoup(plateauCourant,coupAJouer, "X");
                        ...
                        if(branchKey==3) {
                            ArrayList<int[]> possibilities=Possibilities(this.input_board);
                            HashMap<int[],Integer> mapPossibilities=bestPossibilities(this.input_board,possibilities);
                            String [][] plateauSimul;
                            int indice=0;
                            for(int[] key:mapPossibilities.keySet()) {
                                plateauSimul=simulationCoup(this.input_board,key, "O");
                                recursivityLevel=1;
                            ...
                            String [][] plateauSimul;
                            for(int[] key:mapPossibilities.keySet()) {
                                plateauSimul=simulationCoup(plateauCourant,key, "O");
...
*/
    ...
    private String[][] simulateCoup(String[][] board, int[] branchKey, String player)
    {
        board[branchKey[0]]branchKey[1] = player;
        return board;
    }
...
/*
    ...
    private HashMap<int[], Integer> bestPossibilities(String[][] plateaux2, ArrayList<int[]> possibilities, boolean player) {
        HashMap<int[], Integer> accu=new HashMap();
        ArrayList<Integer> res=new ArrayList<>();
        for(int[] coup:possibilities) {
            res.add(evaluationPoint(plateaux2, coup, player));
...
//voir les directions dans evaluation coup plus bas
    public int evaluationDirection(String[][] plateau,int[] coup,String direction, String player) {
            ...
            case "GN":
                ...
                while(place==true) {
                    if(lastX<plateau.length&&lastY>=0) {
                        if(plateau[lastX][lastY].equals(player)) {
...
public HashMap<String, Integer> evaluatePointDirection(String[][] plateau,int[] coup, String direction, String opposite_dir)
{
    HashMap<String, Integer> result=new HashMap<String, Integer>();

    int total_alignement = 1;
    int total_alignement_opponent = 1;
    total_alignement += evaluationDirection(plateau, coup, direction, player) + evaluationDirection(plateau, coup, opposite_dir, player)
    total_alignement_opponent += evaluationDirection(plateau, coup, direction, !player) + evaluationDirection(plateau, coup, opposite_dir, !player)
    result.put("opponent",total_alignement_opponent);
    result.put("player",total_alignement);
    return result;
}

public int evaluatePoint(String[][] plateau,int[] coup,boolean player){
    HashMap<String, Integer> result_player=new HashMap<String, Integer>();
    HashMap<String, Integer> result_opponent=new HashMap<String, Integer>();

    ArrayList<String[]> list_directions = new ArrayList();
    list_directions.add(new String["G", "D"]);
    list_directions.add(new String["GN", "DS"]);
    list_directions.add(new String["DN", "GS"]);
    list_directions.add(new String["N", "S"]);

    for(int i=0; i<list_directions.size(); i++)
    {
        String direction = list_directions.get(i)[0];
        String opposite_direction = list_directions.get(i)[1];

        HashMap<String, Integer> result= evaluatePointDirection(plateau, coup, direction , opposite_direction);
        result_player.put(""+direction + "+" + opposite_direction, result.get("player"));
        result_opponent.put(""+direction + "+" + opposite_direction, result.get("opponent"));
    ...
    public int evaluationPoint(String[][] plateau,int[] coup,boolean joueur){
        ...
        if(joueur) {
            ...
            int totdir4a=1;
            int sommea=0;
            int gaua=evaluationDirection(plateau, coup, "G", false);
            int da=evaluationDirection(plateau, coup, "D", false);
            totdir1a+=gaua+da;
            result.put("G+D",totdir1a);
            sommea+=result.get("G+D");
            gaua=evaluationDirection(plateau, coup, "GN", false);
            da=evaluationDirection(plateau, coup, "DS", false);
            totdir2a+=gaua+da;
            result.put("GS+DS",totdir2a);
            sommea+=totdir2a;
            gaua=evaluationDirection(plateau, coup, "DN", false);
            da=evaluationDirection(plateau, coup, "GS", false);
            totdir3a+=gaua+da;
            result.put("DN+GS",totdir3a);
            sommea+=result.get("DN+GS");
            gaua=evaluationDirection(plateau, coup, "N", false);
            da=evaluationDirection(plateau, coup, "S", false);
            totdir4a+=gaua+da;
            ...
            int totdir4=1;
            int somme=0;
            int gau=evaluationDirection(plateau, coup, "G", true);
            int d=evaluationDirection(plateau, coup, "D", true);
            totdir1+=gau+d;
            result.put("G+D",totdir1);
            somme+=result.get("G+D");
            gau=evaluationDirection(plateau, coup, "GN", true);
            d=evaluationDirection(plateau, coup, "DS", true);
            totdir2+=gau+d;
            result.put("GS+DS",totdir2);
            somme+=totdir2;
            gau=evaluationDirection(plateau, coup, "DN", true);
            d=evaluationDirection(plateau, coup, "GS", true);
            totdir3+=gau+d;
            result.put("DN+GS",totdir3);
            somme+=result.get("DN+GS");
            gau=evaluationDirection(plateau, coup, "N", true);
            d=evaluationDirection(plateau, coup, "S", true);
            totdir4+=gau+d;
    ...
    private void playBestScore(HashMap<int[], Integer> accuValue, String[][] plateaux2) {
        ...
        this.output_board[coupAJouer[0]][coupAJouer[1]]="O";
        String str="";
        for(int i=(this.output_board.length-1);i>=0;i--) {
            for(int j=0;j<this.output_board[i].length;j++) {
                str+=this.output_board[i][j];
    ...
    private boolean gameLostRigth(int i, int j){
        if (j+3 > 6){
            ...
            if(this.input_board[i][j].equals("X") && this.input_board[i][j+1].equals("X") && this.input_board[i][j+2].equals("X") && this.input_board[i][j+3].equals("X")){
                return true;
            }else{
                return false;
            }
    ...
    private boolean gameLostBot(int i, int j){
        if (i-3 < 0){
            ...
            if(this.input_board[i][j].equals("X") && this.input_board[i-1][j].equals("X") && this.input_board[i-2][j].equals("X") && this.input_board[i-3][j].equals("X")){
                return true;
            }else{
                return false;
            }
    ...
    private boolean gameLostLeftBot(int i, int j){
        if ((i-3 < 0)||(j-3<0)){
            ...
            if(this.input_board[i][j].equals("X") && this.input_board[i-1][j-1].equals("X") && this.input_board[i-2][j-2].equals("X") && this.input_board[i-3][j-3].equals("X")){
                return true;
            }else{
                return false;
            }
    ...
    private boolean gameLost() {
        for(int i=0;i<this.input_board.length;i++) {
            for (int j = 0; j < this.input_board[i].length; j++) {
